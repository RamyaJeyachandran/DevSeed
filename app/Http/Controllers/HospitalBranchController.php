...
class HospitalBranchController extends Controller
{
    public function saveHospitalBranch(Request $request)
    {
        try {
            ...
            //----------------Store Image ---Begin 
            $logo =config('constant.hospital_default_logo');
            ...
            $branch = $branch_obj->addHospitalBranch($request);
            $branchId=$branch->id;
             //Login creation
             if($branchId>0){
                ...
                if($password!=NULL){
                    ...
                    if(count($chkEmail)>0){
                        ...
                        $user_obj = new Users;
                        $user_details=$user_obj->createLogin($request,config('constant.branch_user_type_id'),$branchId);
                        $login_created=1;
                    }
                }
            }
    ...
    public function getAllHospitalBranch(Request $request)
    {
        try {
            ...
            $branch_obj = new HospitalBranch;
            $branchList = $branch_obj->getAllHospitalBranch($hospitalId,$pagination);
