             ...
             if($hospitalId>0){
                ...
                if($password!=NULL){
                    ...
                    if(count($chkEmail)>0){
                        ...
                        $user_details=$user_obj->createLogin($request,config('constant.hospital_user_type_id'),$hospitalId);
                        $login_created=1;
                    }
                }
            }
            ...
            if ($validateUser->fails()) {
             ...
             if($request->isImageChanged==1)
             {
                 ...
                 if($request->hasfile('profileImage')){
                     $img_location="images/hospitals/";
                     $img_name =config('constant.prefix_hospital_logo').'_'.time().'.'.$request->profileImage->getClientOriginalExtension();
                     $request->logo->move(public_path($img_location), $img_name);
